...
class NMF:
    ...
    def _init_factors(self, n_users, n_items, global_mean):
        ...
        self.U   = rng.uniform(0, 2 * scale, (n_users, self.n_factors))
        self.V   = rng.uniform(0, 2 * scale, (n_items, self.n_factors))
        self.mu  = global_mean
    ...
    def plot_loss(self):
        ...
        fig.suptitle('NMF — Training History', fontsize=13, fontweight='bold')
    ...
    def predict_test(self, x_test, user_map, item_map,
        ...
        ids, preds = [], []
        for _, row in tqdm(x_test.iterrows(), total=len(x_test), desc='NMF Predicting'):
