...
class SVDpp:
    ...
    def __init__(self, n_factors=50, lambda_U=0.1, lambda_V=0.1,
                 lambda_bu=0.01, lambda_bi=0.01, lambda_Y=0.01,
                 lr=0.01, n_epochs=50, batch_size=1024,
                 clip_min=1.0, clip_max=10.0, random_state=42,
                 optimizer='sgd', huber_delta=1.0, log_every=1,
                 use_biases=True, patience=10):
        ...
        self.U = self.V = self.Y = None
        self.b_u = self.b_i = None
        self.mu  = None
        self.history = []

        self._user_items         = {}
        self._cached_sum_Y       = None
        self._cached_sqrt_norm   = None
    ...
    def _init_factors(self, n_users, n_items, global_mean):
        ...
        self.Y   = rng.normal(0, scale, (n_items, self.n_factors))
        self.mu  = global_mean
        ...
        if self.optimizer == 'adam':
            ...
            self.mY  = np.zeros_like(self.Y);   self.vY  = np.zeros_like(self.Y)
    ...
    def _build_implicit(self, train_users, train_items):
        user_items = defaultdict(list)
        for u, i in zip(train_users, train_items):
            user_items[u].append(i)
        self._user_items = {
            u: np.array(list(set(items)), dtype=np.int32)
            for u, items in user_items.items()
        }

    def _build_implicit_cache(self):
        n_users = self.U.shape[0]
        self._cached_sum_Y     = np.zeros((n_users, self.n_factors))
        self._cached_sqrt_norm = np.zeros(n_users)

        for u, items in self._user_items.items():
            n = len(items)
            if n > 0:
                self._cached_sum_Y[u]     = self.Y[items].sum(axis=0)
                self._cached_sqrt_norm[u] = 1.0 / np.sqrt(n)

    def _get_pu_batch(self, u_idx):
        sqrt_norms = self._cached_sqrt_norm[u_idx]           
        sum_Y_b    = self._cached_sum_Y[u_idx]               
        return self.U[u_idx] + sqrt_norms[:, None] * sum_Y_b 


    def _predict_batch(self, u_idx, i_idx):
        p_u = self._get_pu_batch(u_idx)
        dot = np.sum(p_u * self.V[i_idx], axis=1)
        if self.use_biases:
            return self.mu + self.b_u[u_idx] + self.b_i[i_idx] + dot
        return self.mu + dot

    def _user_vector(self, u_idx):
        sqrt_norm = self._cached_sqrt_norm[u_idx]
        sum_Y     = self._cached_sum_Y[u_idx]
        return self.U[u_idx] + sqrt_norm * sum_Y
    ...
    def _save_state(self):
        state = {'U': self.U.copy(), 'V': self.V.copy(), 'Y': self.Y.copy()}
        if self.use_biases:
            state['b_u'] = self.b_u.copy()
            state['b_i'] = self.b_i.copy()
        return state

    def _load_state(self, state):
        self.U, self.V, self.Y = state['U'], state['V'], state['Y']
        if self.use_biases:
            self.b_u, self.b_i = state['b_u'], state['b_i']

    def fit(self, x_train, y_train, x_val, y_val, user_map, item_map,
            patience=None):
        ...
        self._build_implicit(train_users, train_items)
        self._build_implicit_cache()
        ...
        for epoch in range(self.n_epochs):
            ...
            for start in range(0, n_samples, self.batch_size):
                ...
                V_b   = self.V[i_b]
                p_u_b = self._get_pu_batch(u_b)            
                dot   = np.sum(p_u_b * V_b, axis=1)
                ...
                grad_U = -dgrad[:, None] * V_b   + self.lambda_U  * self.U[u_b]
                grad_V = -dgrad[:, None] * p_u_b + self.lambda_V  * V_b
                ...
                sqrt_norms_b = self._cached_sqrt_norm[u_b] 
                grad_Y_acc   = np.zeros_like(self.Y)        
                count_Y      = np.zeros(len(self.Y))

                for u in np.unique(u_b):
                    items_u = self._user_items.get(u, np.array([], dtype=np.int32))
                    if len(items_u) == 0:
                        continue
                    mask_u  = u_b == u
                    sn      = sqrt_norms_b[mask_u][0]
                    mean_dg = dgrad[mask_u].mean()
                    mean_V  = V_b[mask_u].mean(axis=0)
                    g_y     = -sn * mean_dg * mean_V      

                    grad_Y_acc[items_u]  += g_y[None, :] + self.lambda_Y * self.Y[items_u]
                    count_Y[items_u]     += 1

                nz = count_Y > 0
                if self.optimizer == 'adam':
                    for j in np.where(nz)[0]:
                        g = grad_Y_acc[j] / count_Y[j]
                        self._adam_update(self.Y[j], self.mY[j], self.vY[j], g)
                else:
                    self.Y[nz] -= self.lr * (grad_Y_acc[nz] / count_Y[nz, None])
    ...
    def plot_loss(self):
        ...
        fig.suptitle(f'SVD++ ({bias_str}) — Training History', fontsize=13, fontweight='bold')
        plt.tight_layout()
        plt.show()



    def predict_val(self, x_val, user_map, item_map,
                    user_stats, item_stats, global_mean,
                    cold_unknown_user='global_mean',
                    cold_unknown_item='user_mean',
                    cold_both='const_7',
        ...
        for _, row in x_val.iterrows():
            ...
            item_id = row.iloc[1]
            p = self._predict_one(
                user_id, item_id, user_map, item_map,
                user_stats, item_stats, global_mean,
                cold_unknown_user, cold_unknown_item, cold_both,
                clip_min, clip_max
            )
            preds.append(p)
    ...
    def predict_test(self, x_test, user_map, item_map,
        ...
        for _, row in tqdm(x_test.iterrows(), total=len(x_test), desc='Predicting'):
            ...
            item_id = row.iloc[2]
            ids.append(row_id)
            preds.append(self._predict_one(
                user_id, item_id, user_map, item_map,
                user_stats, item_stats, global_mean,
                cold_unknown_user, cold_unknown_item, cold_both,
                clip_min, clip_max
    ...
    def _predict_one(self, user_id, item_id, user_map, item_map,
                     user_stats, item_stats, global_mean,
                     cold_unknown_user, cold_unknown_item, cold_both,
                     clip_min, clip_max):
        ...
        if not u_known and not i_known:
            return self._cold(cold_both, None, None,
                              user_stats, item_stats, global_mean)
        if not u_known:
            return self._cold(cold_unknown_user, None, item_id,
                              user_stats, item_stats, global_mean)
        if not i_known:
            return self._cold(cold_unknown_item, user_id, None,
                              user_stats, item_stats, global_mean)

        u   = user_map[user_id]
        i   = item_map[item_id]
        p_u = self._user_vector(u)
        dot = float(np.dot(p_u, self.V[i]))

        if self.use_biases:
            p = self.mu + self.b_u[u] + self.b_i[i] + dot
        else:
            p = self.mu + dot
    ...
    @staticmethod
    def _cold(strat, user_id, item_id, user_stats, item_stats, global_mean):
        if strat == 'item_mean' and item_id is not None:
            return float(item_stats.loc[item_id, 'mean']
                         if item_id in item_stats.index else global_mean)
        if strat == 'item_median' and item_id is not None:
            return float(item_stats.loc[item_id, 'median']
                         if item_id in item_stats.index else global_mean)
        if strat == 'user_mean' and user_id is not None:
            return float(user_stats.loc[user_id, 'mean']
                         if user_id in user_stats.index else global_mean)
        if strat == 'user_median' and user_id is not None:
            return float(user_stats.loc[user_id, 'median']
                         if user_id in user_stats.index else global_mean)
        if strat == 'global_mean':
            return float(global_mean)
        if strat.startswith('const_'):
