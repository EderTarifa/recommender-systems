...
class LightFM:
    ...
    def __init__(self, n_factors=800, lambda_U=0.1, lambda_V=0.1,
        ...
        self._user_feat_idx: dict = {}   # user_idx -> np.array de índices en U
        self._item_feat_idx: dict = {}   # item_idx -> np.array de índices en V

        self._n_user_features = 0
        self._n_item_features = 0

    def _build_feature_maps(self, n_users, n_items,
                            user_features=None, item_features=None):
        n_extra_u = 0
        if user_features is not None:
            all_feat_u = set()
            for feats in user_features.values():
                all_feat_u.update(feats)
            n_extra_u = max(all_feat_u) + 1 if all_feat_u else 0

        self._n_user_features = n_users + n_extra_u

        for u in range(n_users):
            feat_list = [u]  
            if user_features is not None and u in user_features:
                for f in user_features[u]:
                    feat_list.append(n_users + int(f))
            self._user_feat_idx[u] = np.array(feat_list, dtype=np.int32)

        n_extra_i = 0
        if item_features is not None:
            all_feat_i = set()
            for feats in item_features.values():
                all_feat_i.update(feats)
            n_extra_i = max(all_feat_i) + 1 if all_feat_i else 0

        self._n_item_features = n_items + n_extra_i

        for i in range(n_items):
            feat_list = [i]  
            if item_features is not None and i in item_features:
                for f in item_features[i]:
                    feat_list.append(n_items + int(f))
            self._item_feat_idx[i] = np.array(feat_list, dtype=np.int32)

    def _init_factors(self, n_users, n_items, global_mean,
                      user_features=None, item_features=None):
        self._build_feature_maps(n_users, n_items, user_features, item_features)

        rng   = np.random.default_rng(self.random_state)
        scale = 1 / np.sqrt(self.n_factors)

        self.U   = rng.normal(0, scale, (self._n_user_features, self.n_factors))
        self.V   = rng.normal(0, scale, (self._n_item_features, self.n_factors))
        self.mu  = global_mean
    ...
    def _embed_user(self, u_idx):
        feats = self._user_feat_idx[u_idx]
        return self.U[feats].mean(axis=0)

    def _embed_item(self, i_idx):
        feats = self._item_feat_idx[i_idx]
        return self.V[feats].mean(axis=0)

    def _predict_batch(self, u_idx, i_idx):
        e_u = np.array([self._embed_user(u) for u in u_idx])   # (batch, K)
        e_i = np.array([self._embed_item(i) for i in i_idx])   # (batch, K)
        dot = np.sum(e_u * e_i, axis=1)
        if self.use_biases:
    ...
    def fit(self, x_train, y_train, x_val, y_val,
        ...
        for epoch in range(self.n_epochs):
            ...
            for start in range(0, n_samples, self.batch_size):
                ...
                e_u_b = np.array([self._embed_user(u) for u in u_b]) 
                e_i_b = np.array([self._embed_item(i) for i in i_b]) 
                dot   = np.sum(e_u_b * e_i_b, axis=1)                 
                ...
                u_feat_grads: dict = {}
                for k, u in enumerate(u_b):
                    feats = self._user_feat_idx[u]
                    n_f   = len(feats)
                    g     = (-dgrad[k] * e_i_b[k] / n_f
                             + self.lambda_U * self.U[feats].mean(axis=0))
                    for f in feats:
                        if f not in u_feat_grads:
                            u_feat_grads[f] = []
                        u_feat_grads[f].append(g)

                i_feat_grads: dict = {}
                for k, i in enumerate(i_b):
                    feats = self._item_feat_idx[i]
                    n_f   = len(feats)
                    g     = (-dgrad[k] * e_u_b[k] / n_f
                             + self.lambda_V * self.V[feats].mean(axis=0))
                    for f in feats:
                        if f not in i_feat_grads:
                            i_feat_grads[f] = []
                        i_feat_grads[f].append(g)

                if self.optimizer == 'adam':
                    for f, grads in u_feat_grads.items():
                        g = np.mean(grads, axis=0)
                        self._adam_update(self.U[f], self.mU[f], self.vU[f], g)
                    for f, grads in i_feat_grads.items():
                        g = np.mean(grads, axis=0)
                        self._adam_update(self.V[f], self.mV[f], self.vV[f], g)
                    if self.use_biases:
                    ...
                    for f, grads in u_feat_grads.items():
                        self.U[f] -= self.lr * np.mean(grads, axis=0)
                    for f, grads in i_feat_grads.items():
                        self.V[f] -= self.lr * np.mean(grads, axis=0)
                    if self.use_biases:
                        for idx in np.unique(u_b):
                            mask = u_b == idx
    ...
    def plot_loss(self):
        ...
        fig.suptitle(f'LightFM ({bias_str}) — Training History', fontsize=13, fontweight='bold')
    ...
    def predict_test(self, x_test, user_map, item_map,
        ...
        for _, row in tqdm(x_test.iterrows(), total=len(x_test), desc='Predicting'):
            ...
            if not u_known and not i_known:
                ...
                u = user_map[user_id]
                i = item_map[item_id]
                e_u = self._embed_user(u)
                e_i = self._embed_item(i)
                if self.use_biases:
                    p = float(self.mu + self.b_u[u] + self.b_i[i] + np.dot(e_u, e_i))
                else:
                    p = float(self.mu + np.dot(e_u, e_i))
